...
import '../../../../../contracts/TartzPool.sol';
...
contract E2E_mint_burn {
    ...
    constructor() public {
        ...
        tartz = new SetupTartz(token0, token1);

        minter = new TartzMinter(token0, token1);
        swapper = new TartzSwapper(token0, token1);
    ...
    function check_mint_invariants(
        int24 _tickLower,
        int24 _tickUpper,
        TartzMinter.MinterStats memory bfre,
        TartzMinter.MinterStats memory aftr
    ) internal {
        (, int24 currentTick, , , , , ) = pool.slot0();
    ...
    function _init(uint128 _seed) internal {
        ...
        // deploy the pool
        //
        tartz.createPool(poolParams.fee, poolParams.startPrice);
        pool = tartz.pool();
    ...
    function test_mint(uint128 _amount) public {
        ...
        (TartzMinter.MinterStats memory bfre, TartzMinter.MinterStats memory aftr) =
    ...
    function test_burn_partial(uint128 _amount) public {
        ...
        try minter.doBurn(pos.tickLower, pos.tickUpper, burnAmount) returns (
            TartzMinter.MinterStats memory bfre_burn,
            TartzMinter.MinterStats memory aftr_burn
        ) {
            bfre = bfre_burn;
            aftr = aftr_burn;
        } catch {
