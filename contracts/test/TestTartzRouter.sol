...
contract TestTartzRouter is ITartzSwapCallback {
    ...
    function tartzV3SwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid =
                amount0Delta > 0 ? ITartzPool(msg.sender).token0() : ITartzPool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == ITartzPool(pools[0]).token1();
            ITartzPool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(ITartzPool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(ITartzPool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,
