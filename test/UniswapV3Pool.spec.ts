...
import { TestTartzSwapPay } from '../typechain/TestTartzSwapPay'
...
import { TestTartzReentrantCallee } from '../typechain/TestTartzReentrantCallee'
...
describe('TartzPool', () => {
  ...
  let factory: TartzFactory
  let pool: MockTimeTartzPool

  let swapTarget: TestTartzCallee
  ...
  async function initializeAtZeroTick(pool: MockTimeTartzPool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  // https://github.com/Tartz/tartz-v3-core/issues/214
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestTartzReentrantCallee')
      ).deploy()) as TestTartzReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestTartzSwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestTartzSwapPay')
      underpay = (await underpayFactory.deploy()) as TestTartzSwapPay
