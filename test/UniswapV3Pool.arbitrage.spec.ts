...
import { TartzPoolSwapTest } from '../typechain/TartzPoolSwapTest'
...
describe('TartzPool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('TartzPoolSwapTest')
            const tester = (await testerFactory.deploy()) as TartzPoolSwapTest
